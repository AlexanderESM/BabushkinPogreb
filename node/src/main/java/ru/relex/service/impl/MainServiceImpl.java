...
public class MainServiceImpl implements MainService {
   // внедряем завимисости
    ...
    public void processTextMessage(Update update) {
        ...
        sendAnswer(output, chatId.toString() );
    }
...
public void processDocMessage(Update update) {
    ...
    try {
        ...
        sendAnswer(answer, chatId.toString());
    } catch (UploadFileException ex) {
        log.error(ex);
        String error = "К сожалению, загрузка файла не удалась. Повторите попытку позже.";
        sendAnswer(error, chatId.toString());
    }
}
    ...
    private boolean isNotAllowToSendContent(Long chatId, AppUser appUser) {
        ...
        if(!appUser.getIsActive()){
            var error ="Зарегестрируйтесь или активируйте свою учётную запись для загрузки контекта";
            sendAnswer(error, chatId.toString());
            return true;
        }else if (!BASIC_STATE.equals(userState)){
    ...
    private String processServiceCommand(AppUser appUser, String cmd) {
        ...
        if(REGISTRATION.equals(serviceCommand)){
            ...
            return "Введена не известная команда. Введите /help";
        }
    }
    ...
    // патерн строитель builder
    private void saveRawData(Update update) {
        RawData rawData = RawData.builder()
                .event(update) // поля которые мы заполняем
                .build(); // в конце прописываем это
